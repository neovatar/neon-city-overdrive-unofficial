...
import { ncouActor } from './documents/actor.mjs';
...
async function createItemMacro(data, slot) {
  ...
  const command = `game.neoncityoverdriveunofficial.rollItemMacro("${data.uuid}");`;
  ...
  if (!macro) {
    macro = await Macro.create({
      ...
      img: item.img,
      command: command,
      flags: { 'neon-city-overdrive-unofficial.itemMacro': true },
    });
  }
